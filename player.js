...
// ─────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────
// ANIMAÇÕES (Motion — motion.dev, via CDN)
// Toda chamada é protegida: se o Motion não carregar por qualquer
// motivo (CDN fora do ar, rede lenta), o site continua funcionando
// normal, só sem esses efeitos extras — nunca trava nada.
// ─────────────────────────────────────────────────────────────
function playEntranceAnimation() {
    if(!window.__motion) return;
    const { animate, stagger } = window.__motion;
    const cards = document.querySelectorAll(
        '.hero-card, .player-clock-card, .player-main-card, .player-suggest-card, .player-admin-link'
    );
...
function animateTrackChange(el) {
    if(!window.__motion || !el) return;
    const { animate } = window.__motion;
    animate(el, { opacity: [0, 1], y: [6, 0] }, { duration: 0.35, easing: [0.16, 1, 0.3, 1] });
}

function animatePlayPress(el) {
    if(!window.__motion || !el) return;
    const { animate } = window.__motion;
...
async function handleSuggestSearch() {
    ...
    try {
        ...
        if(window.__motion) {
            const { animate, stagger } = window.__motion;
            animate(resultsEl.querySelectorAll('.suggest-result-card'),
                { opacity: [0, 1], y: [12, 0] },
                { duration: 0.4, delay: stagger(0.06), easing: [0.16, 1, 0.3, 1] }
            );
        }
