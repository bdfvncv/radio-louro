togglePlayback() {
    ...
    skipTrack() {
        // Em sistema sincronizado, pular não é recomendado pois quebra sincronia
        if (confirm('Pular música quebra a sincronização global. Continuar?')) {
            this.syncToGlobalSchedule();
        }
    }
...
class RadioManager {
    ...
    forceTimeAnnouncement() {
        if (radioState.playlists.time.length > 0) {
            console.log('🕐 Forçando hora certa manualmente');
            const timeTrack = this.getTrackForType('time');
            if (timeTrack) {
                radioState.currentTrack = timeTrack;
                radioState.currentTrackStartTime = new Date().getTime();
                this.updateTrackInfo(timeTrack);
    ...
    handleAudioError() {
        console.error('Erro no áudio, tentando ressincronizar...');
        setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Iniciando Rádio Supermercado do Louro Sincronizada...');
    console.log('Data de início da rádio: 28/09/2024');
    
    radioManager = new SyncedRadioManager();
    radioManager.init();
...
console.log('Sistema de rádio sincronizada carregado!');
        }
